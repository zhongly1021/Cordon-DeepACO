...
class DeepACOAgent:
    ...
    def evaluate_action_log_prob(
        self, env: CordonEnv, heuristic_matrix: torch.Tensor, action: int
    ...
    def rollout(
        self,
        env: CordonEnv,
        heuristic_matrix: torch.Tensor,
        reward_fn: Callable[[List[int]], float | None],
        max_steps: int,
    ) -> Rollout:
        env.reset()
        ...
        for _ in range(max_steps):
            ...
            out = env.step(action)
            raw_reward = reward_fn(out.state)
