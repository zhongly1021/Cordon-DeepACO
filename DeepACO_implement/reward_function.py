...
def _normalize_links(links: Iterable[Tuple]) -> list[tuple[int, int, float, float]]:
    """Support both (u,v,free_time,capacity) and (u,v,length,speed,capacity)."""
    normalized = []
    for row in links:
        if len(row) == 4:
            u, v, free_time, capacity = row
        elif len(row) == 5:
            u, v, length, speed, capacity = row
            free_time = float(length) / max(float(speed), 1e-9)
        else:
            raise ValueError(f"Unsupported link format: {row}")
        normalized.append((int(u), int(v), float(free_time), float(capacity)))
    return normalized
...
class MSARewardFunction:
    """
    Reward by comparing MSA metrics against initial baseline.

    Supports two input styles in `__call__`:
    1) toll dict: {(u, v): toll}
    2) path/state list from CordonEnv, auto-converted to toll dict on visited edges
    """
    ...
    def __init__(
        ...
        fixed_toll: float = 0.5,
    ):
        self.links = _normalize_links(links)
        self.od_init = dict(od_init)
        self.zone_nodes = set(zone_nodes)
        self.solver_kwargs = solver_kwargs or {}
        self.fixed_toll = float(fixed_toll)
        self.initial_metrics: Metrics | None = None

    @staticmethod
    def path_to_toll_dict(path: Sequence[int], fixed_toll: float = 0.5) -> Dict[Tuple[int, int], float]:
        tolls: Dict[Tuple[int, int], float] = {}
        for u, v in zip(path, path[1:]):
            if u == -1 or v == -1:
                continue
            tolls[(int(u), int(v))] = float(fixed_toll)
        return tolls
    ...
    def __call__(self, solution) -> float:
        if self.initial_metrics is None:
            raise RuntimeError("Initial solution not computed. Call compute_initial(...) first.")

        if isinstance(solution, dict):
            toll_dict = solution
        else:
            toll_dict = self.path_to_toll_dict(solution, fixed_toll=self.fixed_toll)
