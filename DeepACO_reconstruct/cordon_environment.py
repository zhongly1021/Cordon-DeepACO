...
class CordonEnv:
    ...
    def __init__(self, road_graph: nx.Graph, max_steps: int = 20, virtual_node: int = -1):
        ...
        self.max_steps = max_steps # number of node
        ...
        for n in self.base_graph.nodes:
            self.graph.add_edge(self.virtual_node, n, is_virtual_edge=True) # 构建edge
    ...
    def available_actions(self) -> List[int]:
        if self.current_node == self.virtual_node:
            return sorted(
                n for n in self.graph.neighbors(self.virtual_node)
                if n != self.virtual_node and n not in self.visited
            )
        real_zone_nodes = [n for n in self.path if n != self.virtual_node]
        #print(real_zone_nodes)
        candidates = set()
        for z in real_zone_nodes:
            for nb in self.graph.neighbors(z):
                candidates.add(nb)
        available = sorted(n for n in candidates if n not in self.visited)
    ...
    def step(self, action: int) -> StepResult:
        ...
        self.current_node = action # should be a list? or visited + current node
